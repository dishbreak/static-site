...
export default function BlogArchive({ loaderData }: Route.ComponentProps): React.JSX.Element {
    return <div className="ml-20 mt-10">
        {loaderData.map(p => <div className="py-4 not-last:border-b border-blue-500">
            <div className="text-sm font-light text-blue-500">{p.posted.toLocaleDateString()}</div>
            <Link to={`/blog/${p.slug}`}>{p.attrs["title"] ?? "(untitled)"}</Link>
        </div>)}
    </div>
}
